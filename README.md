...
```shell
wget -N --no-check-certificate https://raw.githubusercontents.com/Miku-programm/MisakaLinuxToolbox/master/MisakaToolbox.sh && bash MisakaToolbox.sh
```
国内特供版：(部分功能**无法使用**)
```shell
...
```
第一次运行以后，可用快捷方式 ```shell bash MisakaToolbox.sh```启动
